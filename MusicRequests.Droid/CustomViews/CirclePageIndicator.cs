...
namespace MusicRequests.Droid
{
	public class CirclePageIndicator : View, PageIndicator
	{
		...
		public CirclePageIndicator (Context context, IAttributeSet attrs, int defStyle) : base(context, attrs, defStyle)
		{
			...
			mOrientation = a.GetInt (Resource.Styleable.CirclePageIndicator_orientation, 0);
		...
		protected override void OnDraw (Canvas canvas)
		{
			...
			float drawLongFill = longOffset + (mViewPager.CurrentItem * threeRadius);
			if (mOrientation == HORIZONTAL)
			{
				dX = drawLongFill;
				dY = shortOffset;
